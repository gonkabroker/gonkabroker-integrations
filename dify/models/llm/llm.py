...
# Gonka Broker proxy — OpenAI-compatible base URL. The whole point of this branded
# provider is that the endpoint is fixed: users only supply an API key + model id, never
# a base URL. The OpenAI-compatible base class reads `endpoint_url` from credentials and
# joins "chat/completions" onto it, so this must include the "/v1" path segment.
GONKABROKER_ENDPOINT = "https://proxy.gonkabroker.com/v1"


class GonkaBrokerLargeLanguageModel(OAICompatLargeLanguageModel):
    ...
    def _branded_credentials(credentials: Mapping | dict) -> dict:
        """
        Return a copy of the credentials with the Gonka Broker endpoint forced and sane
        OpenAI-compatible defaults applied. The endpoint is never user-editable — that is
        the branding. `mode` defaults to chat (all Gonka models are chat models).
        """
        creds = dict(credentials)
        creds["endpoint_url"] = GONKABROKER_ENDPOINT
        creds.setdefault("mode", "chat")
        return creds
    ...
    def _needs_max_completion_tokens(m: str) -> bool:
        return bool(GonkaBrokerLargeLanguageModel._NEEDS_MAX_COMPLETION_TOKENS_PATTERN.match(m))
    ...
    def get_num_tokens(
        ...
        return super().get_num_tokens(
            model, self._branded_credentials(credentials), prompt_messages, tools
        )
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        """
        credentials = self._branded_credentials(credentials)
        ...
        if agent_thought_support == "supported":
            entity.parameter_rules.append(
                ParameterRule(
                    ...
                    help=I18nObject(
                        en_us="Whether to enable thinking mode, applicable to reasoning models such as MiniMax M2 and Kimi K2.",
                        zh_hans="是否开启思考模式，适用于 MiniMax M2、Kimi K2 等推理模型。",
                    ),
    ...
    def _convert_prompt_message_to_dict(
        ...
        # OpenAI-compatible request shape carries the additional modalities.
    ...
    def _invoke(
        ...
        # Compatibility adapter for Dify's 'json_schema' structured output mode by
        # injecting the JSON schema directly into the system prompt to guide the model.
        ...
        if enable_thinking_value is True and reasoning_effort_value is not None:
            # Propagate reasoning_effort to both top-level and chat_template_kwargs.
        ...
        # (some models do not support server-side thinking control).
    ...
    def _handle_generate_response(
        ...
        Some OpenAI-compatible gateways return tool calls without a `content` field; the SDK
        base class indexes `message["content"]` directly, raising KeyError in that case.
        """
